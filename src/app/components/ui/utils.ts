...
/** Same as getIconShadow but for CSS filter: drop-shadow (no spread). Use for icon-only shadow. */
export function getIconDropShadow(hexColor: string): string {
  const hex = hexColor.replace("#", "");
  if (hex.length !== 6 || !/^[0-9A-Fa-f]{6}$/.test(hex)) {
    return "drop-shadow(0 6px 30px rgba(156, 163, 175, 0.16))";
  }
  const r = parseInt(hex.slice(0, 2), 16);
  const g = parseInt(hex.slice(2, 4), 16);
  const b = parseInt(hex.slice(4, 6), 16);
  return `drop-shadow(0 6px 30px rgba(${r}, ${g}, ${b}, 0.16))`;
}
